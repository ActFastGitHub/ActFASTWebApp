  ...
  useEffect(() => {
    const fetchBoxNames = async () => {
      try {
        ...
        const sortedBoxNames = formattedBoxNames.sort((a: { _count: { name: number; }; }, b: { _count: { name: number; }; }) => b._count.name - a._count.name);
        ...
        {boxNames.map((box) => (
          <li key={box.name}>
            <div className="inline-flex justify-between items-center bg-gray-100 p-2 rounded-md shadow-sm">
              ...
              <span className="bg-blue-500 text-white px-2 py-1 rounded-full ml-2">{box._count.name}</span>
            </div>
          </li>
        ))}
      </ul>
