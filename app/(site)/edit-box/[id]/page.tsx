...
const parseProjectCode = (code?: string) => {
...
const EditBox: React.FC<EditBoxProps> = ({ params }) => {
  ...
  const [previousName, setPreviousName] = useState<string>(""); // NEW
  ...
  const fetchBoxDetails = async () => {
    try {
      ...
      setPreviousName(box.previousName ?? "—"); // NEW
    } catch (err) {
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center bg-gray-200 p-4 pt-16">
      ...
      >
        ...
        {/* Previous Name (read-only) */}
        <div className="mb-4">
          <label className="block text-sm font-medium">Previous Name</label>
          <input
            type="text"
            value={previousName || "—"}
            disabled
