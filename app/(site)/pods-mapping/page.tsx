...
const ClickableGrid: React.FC = () => {
	...
	return (
	  <div className="min-h-screen bg-gray-300 flex flex-col items-center justify-center space-y-4 p-8 relative">
		<select
		  value={currentLevel}
		...
		<div className="flex space-x-12">
		  <div className="flex flex-col space-y-4 mt-1stCol">
			{currentBoxes.slice(0, 5).map((box) => (
			  <Box key={box.id} id={box.id} name={box.name} color={box.color} />
			))}
		  </div>
		  <div className="flex flex-col space-y-4 mt-2ndCol">
			{currentBoxes.slice(5, 8).map((box) => (
			  <Box key={box.id} id={box.id} name={box.name} color={box.color} />
			))}
		  </div>
